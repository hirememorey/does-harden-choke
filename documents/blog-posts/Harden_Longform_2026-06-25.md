...
31 players, ~31,000 games. The cohort spans the spectrum of star scoring archetypes: heliocentric creators (Harden, Luka, Trae), scalable off-ball shooters (Curry, Klay, Ray Allen), rim-pressuring wings (Butler, Giannis), and post-oriented bigs (Embiid, Dirk). Full roster in the project repository.
...
The project pre-registered three hypotheses (variance shift, within-series adaptation, peer comparison) with explicit falsification criteria before any analysis was run. Where the pre-registered tests failed, I report the failures. Where post-hoc extensions produced stronger findings, I label them as such. The full decision log, including every killed hypothesis, is in the open questions document.
...
| SGA | 0% | 35% | Scheme-dependent. Elite defenses take away driving lanes. |
...
Butler and Haliburton have manageable failure modes. A coach can keep Butler engaged or accept that Haliburton will coast in low-stakes games. SGA's floor games cluster against elite defenses, which means scheme adjustments can help. The trigger is identifiable and conditional.
...
*Full code, data pipeline, and decision log at the project repository. This is a working research project. The predictive loop is not closed and the architecture model is descriptive, not predictive. Comments and critique welcome.*
